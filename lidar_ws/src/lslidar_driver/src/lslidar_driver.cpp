...
namespace lslidar_ch_driver {
    ...
    bool LslidarChDriver::loadParameters() {
        ...
        this->declare_parameter<double>("azimuth", 0.0);
        this->declare_parameter<double>("pitch", 0.0);
        this->declare_parameter<double>("position_x", 0.0);
        this->declare_parameter<double>("position_y", 0.0);
        this->declare_parameter<double>("position_z", 0.0);
        ...
        this->get_parameter("azimuth", azimuth);
        ...
        RCLCPP_INFO(this->get_logger(), "pitch: %f rad", pitch);
        RCLCPP_INFO(this->get_logger(), "azimuth: %f rad", azimuth);
        RCLCPP_INFO(this->get_logger(), "position: (%f, %f, %f)", position_x, position_y, position_z);
...
// ch64w
    ...
    int LslidarChDriver::convertCoordinate(struct Firing &lidardata) {
        ...
        // [ralonso] Rotation and Translation -- https://brakeza.com/movimiento-y-rotacion-en-el-espacio-3d/

        //double rads = 10 * DEG_TO_RAD;
        double nx = 1 * x;
        double ny =  cos(pitch) * y + sin(pitch) * z;
        double nz =  -sin(pitch) * y + cos(pitch) * z;
